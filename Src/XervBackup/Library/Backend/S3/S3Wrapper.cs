...
namespace XervBackup.Library.Backend
{
    ...
    public class S3Wrapper : IDisposable
    {
        ...
        public S3Wrapper(string awsID, string awsKey, string locationConstraint, string servername, bool useRRS, bool useSSL)
        {
            ...
            cfg.UserAgent = "XervBackup v" + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString() + " S3 client with AWS SDK v" + cfg.GetType().Assembly.GetName().Version.ToString();
            cfg.UseSecureStringForAwsSecretKey = false;
            cfg.BufferSize = (int)XervBackup.Library.Utility.Utility.DEFAULT_BUFFER_SIZE;
