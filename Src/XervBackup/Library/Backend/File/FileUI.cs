...
namespace XervBackup.Library.Backend
{
    public partial class FileUI : UserControl
    {
        ...
        internal bool Save(bool validate)
        {
            ...
            if (UseCredentials.Checked)
            {
                if (!XervBackup.Library.Backend.File.PreAuthenticate(targetpath, Username.Text, Password.Text))
                {
            ...
            try
            {
                if (!string.IsNullOrEmpty(m_uiAction))
                {
                    if (string.Equals(m_uiAction, "add", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        string[] XervBackup_files = System.IO.Directory.GetFiles(targetpath, "XervBackup-*");

                        if (XervBackup_files.Length > 0)
