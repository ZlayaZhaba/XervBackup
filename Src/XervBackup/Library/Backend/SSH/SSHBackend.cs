    ...
    public class SSH : IBackend_v2, IStreamingBackend, IBackendGUI
    {
            ...
            : this()
        {
            ...
            if (m_options.ContainsKey("transfer-timeout"))
                m_transfer_timeout = Math.Min(1000 * 60 * 60, Math.Max(1000 * 60, (int)XervBackup.Library.Utility.Timeparser.ParseTimeSpan(m_options["transfer-timeout"]).TotalMilliseconds));
            else
