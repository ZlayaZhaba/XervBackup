...
namespace XervBackup.Library.Backend
{
    public class GoogleDocs : IBackend_v2, IStreamingBackend, IBackendGUI
    {
        ...
        private static readonly string USER_AGENT = "XervBackup GoogleDocs backend v" + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString();
                ...
                try
                {
                    ...
                    while (stream.Position != stream.Length)
                    {
                        ...
                        catch (WebException wex)
                        {
							...
							// no other workaround :(
							if (XervBackup.Library.Utility.Utility.IsMono)
							{
								acceptedError |= 
									wex.Status == WebExceptionStatus.ProtocolError &&
                            ...
                            {
                                //Retries are handled in XervBackup, but it is much more efficient here,
