...
namespace XervBackup.Library.DynamicLoader
{
    /// <summary>
        ...
        private List<T> FindInterfaceImplementors(string[] additionalfolders) 
        {
            ...
            foreach (string s in files)
            {
                try
                {
                    ...
                    if (asm != System.Reflection.Assembly.GetExecutingAssembly())
                    {
                        foreach (Type t in asm.GetExportedTypes())
                            ...
                            {
                                XervBackup.Library.Logging.Log.WriteMessage(string.Format(Strings.DynamicLoader.DynamicTypeLoadError, t.FullName, s, ex.ToString()), XervBackup.Library.Logging.LogMessageType.Warning);
                            }
                    }
                }
                catch // (Exception ex)
                {   // Since this is locating the assemblies that have the proper interface, it isn't an error to not.
                    ...
                    // XervBackup.Library.Logging.Log.WriteMessage(string.Format(Strings.DynamicLoader.DynamicAssemblyLoadError, s, ex.ToString()), XervBackup.Library.Logging.LogMessageType.Warning);
                }
            }
