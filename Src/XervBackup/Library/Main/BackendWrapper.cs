...
namespace XervBackup.Library.Main
{
    ...
    internal class BackendWrapper : IDisposable
    {
        ...
        /// </summary>
        private XervBackup.Library.Interface.IBackend m_backend;
        /// <summary>
        ...
        /// <summary>
        /// The set of options used by XervBackup
        /// </summary>
        ...
        /// </summary>
        private XervBackup.Library.Interface.IEncryption m_encryption = null;
        ...
        /// </summary>
        private XervBackup.Library.Interface.IBackendInteraction m_backendInterfaceLogger;
        ...
        private void ResetBackend()
        {
            ...
            if (m_backend == null)
                m_backend = XervBackup.Library.DynamicLoader.BackendLoader.GetBackend(m_backendUrl, m_options.RawOptions);
        }
        ...
        public void AddOrphan(BackupEntryBase entry)
        {
            if (m_orphans != null)
                ...
                Logging.Log.WriteMessage(string.Format(Strings.BackendWrapper.PartialFileFoundMessage, entry.Filename), XervBackup.Library.Logging.LogMessageType.Warning);
        }
            ...
            if (found.Count > 0)
            {
                ...
                if (m_statistics != null)
                {
                    ...
                    switch (m_options.MainAction)
                    {
                        ...
                        case XervBackupOperationMode.DeleteAllButN:
        ...
        public List<ManifestEntry> SortAndPairSets(List<XervBackup.Library.Interface.IFileEntry> files)
        {
            ...
            foreach (XervBackup.Library.Interface.IFileEntry fe in files)
            {
            ...
            foreach (ManifestEntry be in incrementals)
            {
                ...
                {
                    if (m_orphans == null)
                        Logging.Log.WriteMessage(string.Format(Strings.BackendWrapper.OrphanIncrementalFoundMessage, be.Filename), XervBackup.Library.Logging.LogMessageType.Warning);
                    else
                        m_orphans.Add(be);
                    continue;
                }
        ...
        public string DeleteOrphans(bool protectedCleanup)
        {
            ...
            foreach (BackupEntryBase be in m_orphans)
            {
                ...
                Logging.Log.WriteMessage(mfmsg, XervBackup.Library.Logging.LogMessageType.Information);
                sb.AppendLine(mfmsg);
            ...
            {
                Logging.Log.WriteMessage(Strings.BackendWrapper.FilesNotForceRemovedMessage, XervBackup.Library.Logging.LogMessageType.Information);
                sb.AppendLine(Strings.BackendWrapper.FilesNotForceRemovedMessage);
            }
        ...
        private List<XervBackup.Library.Interface.IFileEntry> ListInternal()
        {
            ...
            do
            {
                try
                {
                    ...
                    List<XervBackup.Library.Interface.IFileEntry> res = m_backend.List();
        ...
        private string FindCacheEntry(SignatureEntry remote)
        {
            ...
            if (!System.IO.File.Exists(cachefilename) && System.IO.Directory.Exists(m_options.SignatureCachePath))
                ...
                {
                    BackupEntryBase be = m_cachefilenamestrategy.ParseFilename(new XervBackup.Library.Interface.FileEntry(System.IO.Path.GetFileName(s)));
        ...
        private void PutInternal(BackupEntryBase remote, string filename)
        {
            ...
            XervBackup.Library.Interface.IFileEntry log_fe = new XervBackup.Library.Interface.FileEntry(remote.Filename, remote.Filesize, DateTime.Now, DateTime.Now);
                ...
                do
                {
                    try
                    {
                        ...
                        if (m_backend is Library.Interface.IStreamingBackend && !m_options.DisableStreamingTransfers)
                        {
                            ...
                            using (Utility.ProgressReportingStream pgs = new XervBackup.Library.Utility.ProgressReportingStream(fs, fs.Length))
                            {   
                                pgs.Progress += new XervBackup.Library.Utility.ProgressReportingStream.ProgressDelegate(pgs_Progress);
