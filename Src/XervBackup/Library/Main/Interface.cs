...
using System.Text;
using XervBackup.Library.Utility;

namespace XervBackup.Library.Main
{
    /// <summary>
    /// The operations that XervBackup can report
    /// </summary>
    public enum XervBackupOperation
    {
        /// <summary>
    ...
    /// <summary>
    /// The actual operation that XervBackup supports
    /// </summary>
    public enum XervBackupOperationMode
    {
        /// <summary>
    ...
    /// <summary>
    /// A delegate for reporting progress from within the XervBackup module
    /// </summary>
    ...
    public delegate void OperationProgressEvent(Interface caller, XervBackupOperation operation, XervBackupOperationMode specificoperation, int progress, int subprogress, string message, string submessage);
    ...
    public class Interface : IDisposable, LiveControl.ILiveControl
    {
        ...
        {          
            if (m_options!=null && !m_options.AllowSleep && !XervBackup.Library.Utility.Utility.IsClientLinux)
                try
                {
        ...
        private XervBackupOperation GetOperationType()
        {
            switch (m_options.MainAction)
            {
                case XervBackupOperationMode.Backup:
                case XervBackupOperationMode.BackupFull:
                case XervBackupOperationMode.BackupIncremental:
                    return XervBackupOperation.Backup;
                case XervBackupOperationMode.Restore:
                case XervBackupOperationMode.RestoreControlfiles:
                    return XervBackupOperation.Restore;
                case XervBackupOperationMode.List:
                case XervBackupOperationMode.GetBackupSets:
                case XervBackupOperationMode.ListCurrentFiles:
                case XervBackupOperationMode.ListSourceFolders:
                case XervBackupOperationMode.ListActualSignatureFiles:
                case XervBackupOperationMode.FindLastFileVersion:
                case XervBackupOperationMode.Verify:
                    return XervBackupOperation.List;
                case XervBackupOperationMode.DeleteAllButNFull:
                case XervBackupOperationMode.DeleteOlderThan:
                case XervBackupOperationMode.CleanUp:
                    return XervBackupOperation.Remove;
        ...
        {
            CommunicationStatistics stats = new CommunicationStatistics(XervBackupOperationMode.DeleteAllButN);
            ...
            using (BackendWrapper backend = new BackendWrapper(stats, m_backend, m_options))
                ...
                {
                    if (OperationStarted != null)
                        OperationStarted(this, XervBackupOperation.Remove, stats.OperationMode, 0, -1, Strings.Interface.StatusStarted, "");
                ...
                {
                    if (OperationCompleted != null)
                        OperationCompleted(this, XervBackupOperation.Remove, stats.OperationMode, 100, -1, Strings.Interface.StatusCompleted, "");
                }
        ...
        {
            CommunicationStatistics stats = new CommunicationStatistics(XervBackupOperationMode.DeleteAllButNFull);
            SetupCommonOptions(stats);
        ...
        {
            StringBuilder sb = new StringBuilder();
            CommunicationStatistics stats = new CommunicationStatistics(XervBackupOperationMode.DeleteOlderThan);
            SetupCommonOptions(stats);
        ...
        public string[] List()
        {
            CommunicationStatistics stats = new CommunicationStatistics(XervBackupOperationMode.List);
            SetupCommonOptions(stats);
            ...
            {
                if (OperationStarted != null)
                    OperationStarted(this, XervBackupOperation.List, stats.OperationMode, 0, -1, Strings.Interface.StatusStarted, "");

                foreach (XervBackup.Library.Interface.IFileEntry fe in backend.List(false))
                    res.Add(fe.Name);

                if (OperationCompleted != null)
                    OperationCompleted(this, XervBackupOperation.List, stats.OperationMode, 100, -1, Strings.Interface.StatusCompleted, "");
        ...
        {
            CommunicationStatistics stats = new CommunicationStatistics(XervBackupOperationMode.GetBackupSets);
            SetupCommonOptions(stats);
        ...
        {
            CommunicationStatistics stats = new CommunicationStatistics(XervBackupOperationMode.CreateFolder);
            SetupCommonOptions(stats);
        ...
        {
            if (arg.Type == XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Enumeration)
            {
                bool found = false;
            ...
            else if (arg.Type == XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Boolean)
            {
            ...
            else if (arg.Type == XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Integer)
            {
                long l;
                if (!long.TryParse(value, out l))
                    return string.Format(Strings.Interface.UnsupportedIntegerValue, optionname, value);
            }
            else if (arg.Type == XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Path)
            {
            ...
            else if (arg.Type == XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Size)
            {
                try
                {
            ...
            else if (arg.Type == XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Timespan)
            {
                try
                {
        ...
        public static void RemoveSignatureFiles(string folder)
        {
            ...
            {
                BackupEntryBase e = cachenames.ParseFilename(new XervBackup.Library.Interface.FileEntry(System.IO.Path.GetFileName(s)));
