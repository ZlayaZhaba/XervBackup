...
namespace XervBackup.Library.Main.RSync
{
    /// <summary>
    ...
    public class RSyncDir : IDisposable
    {
        ...
        public enum PatchFileType
        {
            ...
            /// This state is only possible in XervBackup 1.0 archives.
            /// </summary>
            AddedOrUpdatedFile
        };
        ...
        /// Does not create multiple volumes, not used by XervBackup
        /// </summary>
        ...
        private static string GetFullPathFromRelname(string[] sourcefolders, string relpath)
        {
            try
            {
                ...
                else
                {
                    ...
                    //In some versions, XervBackup incorrectly writes the folder name without a trailing slash
        ...
        private System.IO.Stream ProccessDiff(System.IO.Stream fs, string s, Library.Interface.ICompression signaturefile)
        {
            ...
            SharpRSync.ChecksumFileWriter ws = new XervBackup.Library.SharpRSync.ChecksumFileWriter(ms);
        ...
        /// This function is not used by XervBackup.
        /// </summary>
        ...
        public void FinalizeRestore()
        {
                ...
                foreach (string s in m_folders_to_delete)
                {
                    if (SystemIO.DirectoryExists(s))
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Logging.Log.WriteMessage(string.Format(Strings.RSyncDir.DeleteFolderError, s, ex.Message), XervBackup.Library.Logging.LogMessageType.Warning, ex);
                        }
                    else
                        Logging.Log.WriteMessage(string.Format(Strings.RSyncDir.FolderToDeleteMissingError, s), XervBackup.Library.Logging.LogMessageType.Warning);
                }
            ...
            if (m_partialDeltas != null)
            {
                ...
                {
                    Logging.Log.WriteMessage(string.Format(Strings.RSyncDir.PartialFileIncompleteWarning, tf.Key), XervBackup.Library.Logging.LogMessageType.Warning);
                    try
                    {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Logging.Log.WriteMessage(logmessage, XervBackup.Library.Logging.LogMessageType.Error, ex);
                        if (m_stat != null)
            ...
            public bool Callback(string rootpath, string path, FileAttributes attributes)
            {
                ...
                if ((attributes & FileAttributes.ReparsePoint) != 0)
                {
                    ...
                    else if (m_symlinkHandling == Options.SymlinkStrategy.Store)
                    {
                        ...
                        {
                            Logging.Log.WriteMessage(string.Format("Failed to obtain symlink target for {0}, message: {1}", path, ex.Message), XervBackup.Library.Logging.LogMessageType.Warning, ex);
                            if (m_stat != null)
