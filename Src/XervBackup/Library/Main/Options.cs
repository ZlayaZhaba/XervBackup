...
{
    /// <summary>
    /// A class for keeping all XervBackup options in one place,
    ...
    public class Options
    {
        ...
        public IList<ICommandLineArgument> SupportedCommands
        {
            get
            {
                return new List<ICommandLineArgument>(new ICommandLineArgument[] {
                    ...
                    new CommandLineArgument("backup-prefix", CommandLineArgument.ArgumentType.String, Strings.Options.BackupprefixShort, Strings.Options.BackupprefixLong, "XervBackup"),
                    ...
                    new CommandLineArgument("log-file", XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Path, Strings.Options.LogfileShort, Strings.Options.LogfileShort),
                    new CommandLineArgument("log-level", XervBackup.Library.Interface.CommandLineArgument.ArgumentType.Enumeration, Strings.Options.LoglevelShort, Strings.Options.LoglevelLong, "Warning", null, Enum.GetNames(typeof(XervBackup.Library.Logging.LogMessageType))),

                    new CommandLineArgument("verification-level", CommandLineArgument.ArgumentType.Enumeration, Strings.Options.VerificationLevelShort, Strings.Options.VerificationLevelLong, "Manifest", null, Enum.GetNames(typeof(XervBackup.Library.Main.VerificationLevel))),
        ...
        /// </summary>
        public XervBackupOperationMode MainAction 
        {
            get { return (XervBackupOperationMode)Enum.Parse(typeof(XervBackupOperationMode), m_options["main-action"]); }
        ...
        public string BackupPrefix
        {
            get
            {
                if (!m_options.ContainsKey("backup-prefix") || string.IsNullOrEmpty(m_options["backup-prefix"]))
                    return this.UseShortFilenames ? "dpl" : "XervBackup";
                else
        ...
        public Utility.FilenameFilter Filter
        {
            get
            {
                if (m_options.ContainsKey("filter") && !string.IsNullOrEmpty(m_options["filter"]))
                    return new XervBackup.Library.Utility.FilenameFilter(Utility.FilenameFilter.DecodeFilter(m_options["filter"]));
                else
                    return new XervBackup.Library.Utility.FilenameFilter(new List<KeyValuePair<bool, string>>());
            }
        }
        ...
        /// </summary>
        public XervBackup.Library.Logging.LogMessageType Loglevel
        {
            get
            {
                string value;
                ...
                foreach (string s in Enum.GetNames(typeof(XervBackup.Library.Logging.LogMessageType)))
                    if (s.Equals(value, StringComparison.InvariantCultureIgnoreCase))
                        return (XervBackup.Library.Logging.LogMessageType)Enum.Parse(typeof(XervBackup.Library.Logging.LogMessageType), s);

                return XervBackup.Library.Logging.LogMessageType.Warning;
            }
        }
        ...
        /// </summary>
        public XervBackup.Library.Main.VerificationLevel Verificationlevel
        {
            get
            {
                string value;
                ...
                foreach (string s in Enum.GetNames(typeof(XervBackup.Library.Main.VerificationLevel)))
                    if (s.Equals(value, StringComparison.InvariantCultureIgnoreCase))
                        return (XervBackup.Library.Main.VerificationLevel)Enum.Parse(typeof(XervBackup.Library.Main.VerificationLevel), s);

                return XervBackup.Library.Main.VerificationLevel.Manifest;
            }
        }
