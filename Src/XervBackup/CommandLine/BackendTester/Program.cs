...
namespace XervBackup.CommandLine.BackendTester
{
    class Program
    {
        class TempFile
        ...
        {
            Console.Write("Uploading file {0}, {1} ... ", i, XervBackup.Library.Utility.Utility.FormatSizeString(new System.IO.FileInfo(localfilename).Length));
            Exception e = null;
        ...
        private static string CreateRandomFile(Library.Utility.TempFolder tf, int i, int min_file_size, int max_file_size, Random rnd)
        {
            ...
            using (System.IO.FileStream fs = new System.IO.FileStream(filename, System.IO.FileMode.CreateNew, System.IO.FileAccess.Write))
            {
                ...
                Console.WriteLine(" ({0})", XervBackup.Library.Utility.Utility.FormatSizeString(size));
