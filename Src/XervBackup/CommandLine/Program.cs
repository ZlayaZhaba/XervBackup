...
namespace XervBackup.CommandLine
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                string filter = XervBackup.Library.Utility.FilenameFilter.EncodeAsFilter(XervBackup.Library.Utility.FilenameFilter.ParseCommandLine(cargs, true));
                ...
                if (source.Trim().ToLower() == "list")
                    Console.WriteLine(string.Join("\r\n", XervBackup.Library.Main.Interface.List(target, options)));
                else if (source.Trim().ToLower() == "list-current-files")
                {
                    ...
                    Console.WriteLine(string.Join("\r\n", new List<string>(XervBackup.Library.Main.Interface.ListCurrentFiles(target, options)).ToArray()));
                }
                else if (source.Trim().ToLower() == "list-source-folders")
                {
                    ...
                    Console.WriteLine(string.Join("\r\n", XervBackup.Library.Main.Interface.ListSourceFolders(target, options) ?? new string[0]));
                }
                else if (source.Trim().ToLower() == "list-actual-signature-files")
                {
                    ...
                    List<KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string>> files = XervBackup.Library.Main.Interface.ListActualSignatureFiles(cargs[0], options);

                    Console.WriteLine("* " + Strings.Program.DeletedFoldersHeader + ":");
                    foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                        if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.DeletedFolder)
                            Console.WriteLine(x.Value);

                    Console.WriteLine();
                    Console.WriteLine("* " + Strings.Program.AddedFoldersHeader + ":");
                    foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                        if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.AddedFolder)
                            Console.WriteLine(x.Value);

                    Console.WriteLine();
                    Console.WriteLine("* " + Strings.Program.DeletedFilesHeader + ":");
                    foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                        if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.DeletedFile)
                            Console.WriteLine(x.Value);

                    bool hasCombinedSignatures = false;
                    foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                        if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.AddedOrUpdatedFile)
                        {
                    ...
                    else
                    {
                        ...
                        foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                            if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.AddedFile)
                                Console.WriteLine(x.Value);

                        Console.WriteLine();
                        Console.WriteLine("* " + Strings.Program.ModifiedFilesHeader + ":");
                        foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                            if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.UpdatedFile)
                                Console.WriteLine(x.Value);
                    }
                    ...
                    foreach (KeyValuePair<XervBackup.Library.Main.RSync.RSyncDir.PatchFileType, string> x in files)
                        if (x.Key == XervBackup.Library.Main.RSync.RSyncDir.PatchFileType.ControlFile)
                            Console.WriteLine(x.Value);
                }
                else if (source.Trim().ToLower() == "collection-status")
                {
                    ...
                    List<XervBackup.Library.Main.ManifestEntry> entries = XervBackup.Library.Main.Interface.ParseFileList(cargs[0], options);
                    
                    Console.WriteLine(Strings.Program.CollectionStatusHeader.Replace("\\t", "\t"), entries.Count);
                    
                    foreach (XervBackup.Library.Main.ManifestEntry m in entries)
                    {
                        Console.WriteLine();

                        long size = Math.Max(m.Fileentry.Size ,0);
                        foreach (KeyValuePair<XervBackup.Library.Main.SignatureEntry, XervBackup.Library.Main.ContentEntry> x in m.Volumes)
                        ...
                        foreach (XervBackup.Library.Main.ManifestEntry mi in m.Incrementals)
                        {
                            size = Math.Max(mi.Fileentry.Size, 0);
                            foreach (KeyValuePair<XervBackup.Library.Main.SignatureEntry, XervBackup.Library.Main.ContentEntry> x in mi.Volumes)
                                size += Math.Max(x.Key.Fileentry.Size, 0) + Math.Max(x.Value.Fileentry.Size, 0);
                ...
                else if (source.Trim().ToLower() == "delete-all-but-n-full" || source.Trim().ToLower() == "delete-all-but-n")
                {
                    ...
                    if (source.Trim().ToLower() == "delete-all-but-n")
                        Console.WriteLine(XervBackup.Library.Main.Interface.DeleteAllButN(cargs[0], options));
                    else
                        Console.WriteLine(XervBackup.Library.Main.Interface.DeleteAllButNFull(cargs[0], options));
                }
                else if (source.Trim().ToLower() == "delete-older-than")
                {
                    try
                    {
                        XervBackup.Library.Utility.Timeparser.ParseTimeSpan(target);
                    }
                    catch (Exception ex)
                    {
                    ...
                    Console.WriteLine(XervBackup.Library.Main.Interface.DeleteOlderThan(cargs[0], options));
                }
                else if (source.Trim().ToLower() == "cleanup")
                {
                    ...
                    Console.WriteLine(XervBackup.Library.Main.Interface.Cleanup(cargs[0], options));
                }
                else if (source.Trim().ToLower() == "create-folder")
                {
                    ...
                    XervBackup.Library.Main.Interface.CreateFolder(cargs[0], options);
                ...
                else if (source.Trim().ToLower() == "find-last-version")
                {
                    ...
                    List<KeyValuePair<string, DateTime>> results = XervBackup.Library.Main.Interface.FindLastFileVersion(cargs[0], options);
                ...
                else if (source.Trim().ToLower() == "verify")
                {
                    ...
                    List<KeyValuePair<XervBackup.Library.Main.BackupEntryBase, Exception>> results = XervBackup.Library.Main.Interface.VerifyBackup(cargs[0], options);
                    ...
                    {
                        if (x.Key is XervBackup.Library.Main.ManifestEntry)
                            manifests++;
                        else if (x.Key is XervBackup.Library.Main.SignatureEntry)
                            signatures++;
                        else if (x.Key is XervBackup.Library.Main.ContentEntry)
                            contentfiles++;
                ...
                else if (source.IndexOf("://") > 0 || options.ContainsKey("restore"))
                {
                    ...
                    Console.WriteLine(XervBackup.Library.Main.Interface.Restore(source, target.Split(System.IO.Path.PathSeparator), options));
                }
                else
                {
                    if (cargs.Count != 2)
                    ...
                    Console.WriteLine(XervBackup.Library.Main.Interface.Backup(source.Split(System.IO.Path.PathSeparator), target, options));
                }
            }
            catch (Exception ex)
            {
        ...
        private static bool ReadOptionsFromFile(string filename, ref string filter, IList<string> cargs, Dictionary<string, string> options)
        {
            try
            {
                ...
                string newfilter = XervBackup.Library.Utility.FilenameFilter.EncodeAsFilter(XervBackup.Library.Utility.FilenameFilter.ParseCommandLine(fargs, true));
