    ...
    public abstract class BackupTask : IDuplicityTask
    {
        ...
        protected virtual string SetupBackend(Dictionary<string, string> environment, Dictionary<string, string> options)
        {
            ...
            else
            {
                ...
                if (!options.ContainsKey(XervBackup.GUI.Wizard_pages.GridContainer.DESTINATION_EXTENSION_KEY))
                    throw new Exception("Invalid configuration");

                destination = options[XervBackup.GUI.Wizard_pages.GridContainer.DESTINATION_EXTENSION_KEY];
        ...
        protected virtual Log WriteLogMessage(string action, string subaction, string data, string parsedMessage, bool commit, Log log)
        {
            lock (Program.MainLock)
            {
                ...
                XervBackupOutputParser.ParseData(log);
                ...
                {
                    log.ParsedStatus = XervBackupOutputParser.InterruptedStatus;
                    log.ParsedMessage = "";
                }
