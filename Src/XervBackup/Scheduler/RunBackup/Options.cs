...
{
    /// <summary>
    /// Manipulate Options to be XervBackup ready
    /// </summary>
    public class Options : Dictionary<string, string>
    {
        ...
        /// <summary>
        /// Parse Options ready for XervBackup backup run
        /// </summary>
        ...
        public Options(string aJob, string aXML)
        {
            ...
            using (XervBackup.Scheduler.Data.SchedulerDataSet sds = new XervBackup.Scheduler.Data.SchedulerDataSet())
            {
                // Let this throw...
                Exception Ex = sds.Load(aXML);  // Load database from XML
                if (Ex != null) Library.Logging.Log.WriteMessage(aXML + ":" + Ex.Message, XervBackup.Library.Logging.LogMessageType.Error);
                // Fetch turned off monitors
                this.DisabledMonitors = sds.Settings.DisabledMonitors;
                // Find the job
                XervBackup.Scheduler.Data.SchedulerDataSet.JobsRow Row = sds.Jobs.FindByName(aJob);
                ...
                if (Row.MapDrives)
                {
                    foreach (XervBackup.Scheduler.Data.SchedulerDataSet.DriveMapsRow dRow in Row.GetDriveMapsRows())
                    {
                        System.IO.DriveInfo di = new System.IO.DriveInfo(dRow.DriveLetter);
                        if (di != null && !di.IsReady)
                            MapResults += XervBackup.Scheduler.Utility.User.Run("net.exe", "use " + di.Name + ' ' + dRow.UNC) + '\n';
                    }
                }
            ...
            this["log-level"] = XervBackup.Library.Logging.Log.LogLevel.ToString(); // Don't mess with the log level.
            // uh hmmm
            if (this.ContainsKey("Checksum"))
            {
                ...
                this["passphrase"] = System.Text.ASCIIEncoding.ASCII.GetString(XervBackup.Scheduler.Utility.Tools.Unprotect(this.Checksum));
        ...
        public static bool IsFull(string aJob, int aNumberRuns, int aRepeatDays)
        {
            ...
            try
            {
                ...
                XervBackup.Scheduler.Data.HistoryDataSet hds = new XervBackup.Scheduler.Data.HistoryDataSet();
                hds.Load();
        ...
        /// <summary>
        /// Turns filters in the "[+-]filter;..." format to XervBackup "[ei]:filter;..." format
        /// </summary>
        /// <param name="aFilterString">"[+-]filter;..." formatted filters</param>
        /// <returns>XervBackup "[ei]:filter;..." format filters</returns>
