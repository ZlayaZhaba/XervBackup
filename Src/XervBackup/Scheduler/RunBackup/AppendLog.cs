...
namespace XervBackup.Library.Logging
{
    /// <summary>
    ...
    public class AppendLog : ILog, IDisposable
    {
        ...
        public class LogEntry
        {
            ...
            public XervBackup.Library.Logging.LogMessageType Type { get; set; }
            public string Message { get; set; }
            public string ExMessage { get; set; }
            public LogEntry(string aLine)
            {
                string[] Parts = aLine.Split(new string[] { XervBackup.Library.Logging.AppendLog.Separator }, StringSplitOptions.None);
                if (Parts.Length < 5) return;
                Date = XervBackup.Library.Logging.AppendLog.ParseLogDate(Parts[0]);
                Job = Parts[1];
                Type = (XervBackup.Library.Logging.LogMessageType)System.Enum.Parse(typeof(XervBackup.Library.Logging.LogMessageType), Parts[2]);
                Message = Parts[3];
                ExMessage = Parts[4];
            }
        }
        ...
        /// </summary>
        public class LogList : System.ComponentModel.BindingList<XervBackup.Library.Logging.AppendLog.LogEntry>
        {
            public LogList(string[] aContents)
                : base((from string qS in aContents select new XervBackup.Library.Logging.AppendLog.LogEntry(qS)).ToArray())
            {
            }
        }
        #region ILog Members
        ...
        public virtual void WriteMessage(string aMessage, XervBackup.Library.Logging.LogMessageType aType, Exception aException)
        {
