...
namespace XervBackup.Scheduler
{
    ...
    public partial class Scheduler : Form
    {
        ...
        /// </summary>
        private XervBackup.Scheduler.Data.SchedulerDataSet.JobsRow CurrentRow
        {
            get
            {
                if (this.JobsBindingSource.Current == null) return null;
                return (XervBackup.Scheduler.Data.SchedulerDataSet.JobsRow)((DataRowView)this.JobsBindingSource.Current).Row;
            }
        }
        /// <summary>
        ...
        private void SelectToolStripDropDownButton_DropDownOpening(object sender, EventArgs e)
        {
            ...
            this.SelectToolStripDropDownButton.DropDownItems.AddRange(
                (from XervBackup.Scheduler.Data.SchedulerDataSet.JobsRow qR in this.SchedulerDataSet.Jobs.Select()
        ...
        private void OperationProgress(string aMessage)
        {
            ...
            // Decode the message
            XervBackup.Scheduler.RunBackup.Pipe.ProgressArguments pa = new XervBackup.Scheduler.RunBackup.Pipe.ProgressArguments(aMessage);
        ...
        private void SettingsToolStripDropDownButton_DropDownOpening(object sender, EventArgs e)
        {
            ...
            {
                // Look for plug in's
                string[] Plugins = System.IO.Directory.GetFiles(Application.StartupPath, "XervBackup.Scheduler.Monitor.*.dll");
                ...
                else
                {
                    ...
                    foreach (string Plug in Plugins)
                    {
                        ...
                        if (Ass != null)
                        {
                            ...
                            XervBackup.Scheduler.Data.IMonitorPlugin Monitor = Activator.CreateInstance(ClassType, null) 
                                as XervBackup.Scheduler.Data.IMonitorPlugin;
        ...
        private void tsi_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            XervBackup.Scheduler.Data.IMonitorPlugin Mon = (XervBackup.Scheduler.Data.IMonitorPlugin)tsi.Tag;
