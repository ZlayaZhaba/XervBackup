...
namespace XervBackup.Scheduler
{
    ...
    public partial class FileListDialog : Form
    {
        ...
        /// <param name="aOptions">Used by XervBackup</param>
        ...
        /// <summary>
        /// Get the source folders from XervBackup
        /// </summary>
        /// <param name="aURI">Source</param>
                ...
                Utility.Tools.Background((Action)delegate()
                    {
                        try
                        {
                            Result = XervBackup.Library.Main.Interface.ListSourceFolders(aURI, FixOptions(aOptions));
                        }
        ...
        /// <summary>
        /// Ask XervBackup to list the backup's files
        /// </summary>
        /// <param name="aURI">Source</param>
        /// <param name="aOptions">XervBackup options</param>
        ...
        private List<string> ListCurrentFiles(string aURI, Dictionary<string, string> aOptions, string[] aSourceFolders)
        {
            ...
            Utility.Tools.Background((Action)
                delegate() 
                { 
                    Result = new List<string>(XervBackup.Library.Main.Interface.ListCurrentFiles(aURI, FixOptions(aOptions))); 
                } );
            // If there is only one source folder, XervBackup does not prepend indexes, so add them manually
        ...
        private void MakeTree(string aURI, Dictionary<string, string> aOptions)
        {
            ...
            // Basically stolen from XervBackup
            try
            {
