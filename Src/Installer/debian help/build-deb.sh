...
UNWANTED_FILES="sqlite3.dll win-tools sqlite-3.6.12.so StartXervBackup.sh"
...
if [ -n "$SHOW_USAGE_ERROR" ]; then
	...
	echo
	echo "Usage:"
	echo "$0 \"XervBackup 1.3.zip\" [output.deb]"
	echo
	exit
fi
...
VERSION=`basename "$1"`
VERSION=${VERSION%%.zip}
VERSION=${VERSION##XervBackup }

VERSION_FILENAME="${VERSION// /-}"
VERSION_FILENAME="${VERSION_FILENAME//(/}"
VERSION_FILENAME="${VERSION_FILENAME//)/}"
ROOT_DIR="XervBackup_$VERSION_FILENAME"
BUILD_TIME=`date -R`
PWD=`pwd`
...
OUTPUT_RPM="XervBackup-$VERSION_FILENAME-1.noarch.rpm"
OUTPUT_SLP="XervBackup-$VERSION_FILENAME.slp"
OUTPUT_LSB="lsb-XervBackup-$VERSION_FILENAME-1.noarch.rpm"
OUTPUT_TGZ="XervBackup-$VERSION_FILENAME.tgz"
...
if [ -d "XervBackup-$VERSION_FILENAME" ]; then
	rm -rf "XervBackup-$VERSION_FILENAME"
fi
...
do
	if [ -e "XervBackup/$FILE" ]
	then
		rm -rf "XervBackup/$FILE"
	fi
done
...
mkdir "$ROOT_DIR/usr/share/doc/XervBackup"

# Place binary contents in the lib/XervBackup dir
mv "XervBackup" "$ROOT_DIR/usr/lib/XervBackup"

# Insert pacakage control files
cp -R DEBIAN "$ROOT_DIR/"
rm -rf "$ROOT_DIR/DEBIAN/.svn"
mv "$ROOT_DIR/usr/lib/XervBackup/linux-readme.txt" "$ROOT_DIR/usr/share/doc/XervBackup/README"
echo "This package was debianized by Xervbackup <opensource@hexad.dk> on $BUILD_TIME." > "$ROOT_DIR/usr/share/doc/XervBackup/copyright"
cat "copyright" >> "$ROOT_DIR/usr/share/doc/XervBackup/copyright"

# Generate a plausible changelog
echo "XervBackup ($VERSION_FILENAME) unstable; urgency=low" > "$ROOT_DIR/usr/share/doc/XervBackup/changelog"
echo "" >> "$ROOT_DIR/usr/share/doc/XervBackup/changelog"
echo "  * XervBackup binary release $VERSION" >> "$ROOT_DIR/usr/share/doc/XervBackup/changelog"
echo "" >> "$ROOT_DIR/usr/share/doc/XervBackup/changelog"
echo " -- Xervbackup <opensource@hexad.dk>  $BUILD_TIME" >> "$ROOT_DIR/usr/share/doc/XervBackup/changelog"
echo "" >> "$ROOT_DIR/usr/share/doc/XervBackup/changelog"

cp "$ROOT_DIR/usr/share/doc/XervBackup/changelog" "$ROOT_DIR/usr/share/doc/XervBackup/changelog.Debian"

gzip --quiet --best "$ROOT_DIR/usr/share/doc/XervBackup/changelog"
gzip --quiet --best "$ROOT_DIR/usr/share/doc/XervBackup/changelog.Debian"
...
chmod +x "$ROOT_DIR/usr/bin/XervBackup"
chmod +x "$ROOT_DIR/usr/bin/XervBackup-commandline"
