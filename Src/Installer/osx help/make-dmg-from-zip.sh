...
SOURCE_FILES=XervBackup.app
OUTPUT=XervBackup.dmg
...
then
	...
	echo
	echo "Usage:"
	echo "$0 \"XervBackup 1.3.zip\" [output.dmg]"
	echo
	exit
fi
...
if [ ! -d "XervBackup" ]
then
	echo "After unzipping the file $1 there was no XervBackup directory, is the zip file correct?"
	exit
fi
...
# Make the XervBackup.app structure, root folder should exist
if [ ! -d "$WC_DIR/XervBackup.app" ]
then
	mkdir "$WC_DIR/XervBackup.app"
fi

mkdir "$WC_DIR/XervBackup.app/Contents"
mkdir "$WC_DIR/XervBackup.app/Contents/Resources"
mkdir "$WC_DIR/XervBackup.app/Contents/MacOS"

# Copy in contents
cp "Info.plist" "$WC_DIR/XervBackup.app/Contents/"
cp "XervBackup-launcher" "$WC_DIR/XervBackup.app/Contents/MacOS/XervBackup"
cp -R "./XervBackup/"* "$WC_DIR/XervBackup.app/Contents/Resources"
cp "XervBackup.icns" "$WC_DIR/XervBackup.app/Contents/Resources"
chmod +x "$WC_DIR/XervBackup.app/Contents/MacOS/XervBackup"
