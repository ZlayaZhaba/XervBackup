    ...
    static class Program
    {
        ...
        private static void Export(string cultures)
        {
            ...
            foreach (string culture in GetLocaleFolders(cultures))
            {
                ...
                {
                    string filename = file.Attribute("filename").Value.Substring(XervBackup.Library.Utility.Utility.AppendDirSeparator(Application.StartupPath).Length);
        ...
        private static void ExportDiff(string culture, string inputfile)
        {
            ...
            if (currentFile.Equals(inputfile, XervBackup.Library.Utility.Utility.ClientFilenameStringComparision))
        ...
        private static void WriteCSVFile(string filename, string culture, Dictionary<string, Dictionary<string, CSVEntry>> entries, Dictionary<string, Dictionary<string, string>> status_overrides)
        {
            ...
            using (System.IO.StreamWriter sw = new System.IO.StreamWriter(filename, false, new System.Text.UTF8Encoding(false)))
            {
                ...
                foreach (KeyValuePair<string, KeyValuePair<string, CSVEntry>> f in sortedlist)
                {
                    ...
                    string path = f.Key;
                    if (f.Key.StartsWith(filenameprefix, XervBackup.Library.Utility.Utility.ClientFilenameStringComparision))
        ...
        private static void Report(string cultures)
        {
            foreach (string culture in GetLocaleFolders(cultures))
            {
                ...
                var extraFiles = from x in XervBackup.Library.Utility.Utility.EnumerateFiles(System.IO.Path.Combine(Application.StartupPath, culture))
                                 where 
        ...
        public static IEnumerable<ResXFileInfo> GetResXList(string culture)
        {
            ...
            foreach (XElement conf in XDocument.Load(System.IO.Path.Combine(Application.StartupPath, "configuration.xml")).Element("root").Elements("configuration"))
            {
				...
				string outputfolder = System.IO.Path.GetFullPath(XervBackup.Library.Utility.Utility.AppendDirSeparator(System.IO.Path.Combine(Application.StartupPath, culture)));
                sourcefolder = XervBackup.Library.Utility.Utility.AppendDirSeparator(System.IO.Path.GetFullPath(sourcefolder));
                ...
                foreach (XElement fn in conf.Elements("assembly"))
                {
                    ...
                    foreach (string s in XervBackup.Library.Utility.Utility.EnumerateFiles(System.IO.Path.Combine(sourcefolder, subfolder)))
                    {
            ...
            foreach (string culture in GetLocaleFolders(cultures))
            {
                foreach (ResXFileInfo inf in GetResXList(culture))
                {
                                           ...
                                           )
                    {
						...
						{
							if (XervBackup.Library.Utility.Utility.IsClientLinux && !System.IO.File.Exists(item.sourceRes)) 
							{
        ...
        {
            string root = XervBackup.Library.Utility.Utility.AppendDirSeparator(Application.StartupPath);
            if (!string.IsNullOrEmpty(cultureReq))
                root = System.IO.Path.Combine(root, cultureReq);

            foreach (string s in XervBackup.Library.Utility.Utility.EnumerateFiles(Application.StartupPath))
